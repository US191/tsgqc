...
switch length(varargin)
  ...
  otherwise 
    error('tsgqc:netcdf:write', 'one or two arg needed');
...
switch fileExt
  ...
  case {'.csv', '.json'}
    ...
    % ------------------------------------------------
    write@tsgqc.dynaload(self);
    return
    
  case {'.xls', '.xlsx'}
    error('tsgqc:netcdf:write', ['Save NetCDF in Excel format.\n',...
...
%     end
  ...
  try
    ...
    % 'Matlab:tsgqc.netcdf.write:netcdf.defVar',...
    error('tsgqc:netcdf:write', ...
  ...
  try
    ...
    % 'Matlab:tsgqc.netcdf.write:netcdf.putVar'
    error('tsgqc:netcdf:write', ...
